...
import { Search, Filter, Calendar, DollarSign, GraduationCap, MapPin, ExternalLink, X, FileText } from "lucide-react"
...
export default function ScholarshipsPage() {
  ...
  const filteredScholarships = scholarships.filter(scholarship => {
    ...
    const matchesAmountType = selectedAmountType === "all" || scholarship.amount_type === selectedAmountType
    
    return matchesSearch && matchesCategory && matchesLevel && matchesAmountType
  })
  ...
  const hasActiveFilters = searchTerm !== "" || selectedCategory !== "all" || 
    selectedLevel !== "all" || selectedAmountType !== "all"
    ...
    <main className="min-h-screen bg-gray-50">
      ...
      <nav className="bg-white shadow-sm px-8 py-4 flex items-center justify-between">
        ...
        <div className="flex gap-6">
          <Link href="/careers" className="text-gray-600 hover:text-indigo-600 text-sm">
            Careers
          </Link>
          <Link href="/colleges" className="text-gray-600 hover:text-indigo-600 text-sm">
            Colleges
          </Link>
          <Link href="/exams" className="text-gray-600 hover:text-indigo-600 text-sm">
            Exams
          </Link>
        </div>
      </nav>
      ...
      <div className="bg-gradient-to-r from-emerald-600 to-teal-500 px-8 py-12 text-white">
        <h1 className="text-4xl font-extrabold mb-2">Scholarships & Financial Aid</h1>
        <p className="text-emerald-100 text-lg">{scholarships.length}+ scholarships to help fund your education</p>
        {/* Stats */}
      ...
      <div className="max-w-7xl mx-auto px-8 py-10">
        ...
        <div className="flex items-center justify-between mb-6">
          <p className="text-gray-600">
            Showing <span className="font-semibold text-gray-900">{filteredScholarships.length}</span> of <span className="font-semibold text-gray-900">{scholarships.length}</span> scholarships
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredScholarships.map((scholarship) => {
            ...
            const deadlineSoon = isDeadlineSoon(scholarship.application_deadline)
            ...
            return (
              ...
              >
                ...
                {/* Name & Provider */}
                <Link href={`/scholarships/${scholarship.slug}`} className="group">
                ...
                <div className="bg-emerald-50 rounded-xl p-3 mb-3">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="font-semibold text-emerald-700">{formatAmount(scholarship)}</span>
                  </div>
                </div>
                ...
                <div className="space-y-2 mb-4">
                  <div className="flex items-center gap-2 text-sm text-gray-600">
                    ...
                    <span className="capitalize">{scholarship.level.replace('_', ' ').replace(/\b\w/g, (l: string) => l.toUpperCase())}</span>
                  </div>
                  {scholarship.state && (
                    <div className="flex items-center gap-2 text-sm text-gray-600">
                      <MapPin className="w-4 h-4" />
                      <span>{scholarship.state}, {scholarship.country}</span>
                    </div>
                  )}
                  <div className="flex items-center gap-2 text-sm text-gray-600">
                    <Calendar className="w-4 h-4" />
                    <span className={deadlineSoon ? "text-red-600 font-medium" : ""}>
