...
export default function AdminCollegesPage() {
  ...
  const def = RESOURCES.colleges

  const load = async (search = q) => {
    setLoading(true)
    const params = new URLSearchParams({ table: 'colleges' })
    ...
    <div>
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">Create, edit, or remove colleges shown to students.</p>
        </div>
        <Link
            ...
            {loading ? (
              ...
              <tr key={String(row.id)} className="border-t border-gray-100 hover:bg-gray-50">
                ...
                <td className="px-4 py-3">
                  <Link href={`/admin/colleges/${row.id}`} className="text-indigo-600 hover:underline">
                    Edit
                  </Link>
                </td>
              </tr>
