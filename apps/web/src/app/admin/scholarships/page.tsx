...
export default function AdminScholarshipsPage() {
  ...
  const def = RESOURCES.scholarships

  const load = async (search = q) => {
    setLoading(true)
    const params = new URLSearchParams({ table: 'scholarships' })
    ...
    <div>
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">Create, edit, or remove scholarships shown to students.</p>
        </div>
        <Link
            ...
            {loading ? (
              ...
              <tr key={String(row.id)} className="border-t border-gray-100 hover:bg-gray-50">
                ...
                <td className="px-4 py-3">
                  <Link href={`/admin/scholarships/${row.id}`} className="text-indigo-600 hover:underline">
                    Edit
                  </Link>
                </td>
              </tr>
