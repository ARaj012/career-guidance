...
export default function AdminBlogPage() {
  ...
  const def = RESOURCES.blog

  const load = async (search = q) => {
    setLoading(true)
    const params = new URLSearchParams({ table: 'blog_posts' })
    ...
    <div>
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">Create, edit, or remove blog posts shown to students.</p>
        </div>
        <Link
          href="/admin/blog/new"
            ...
            {loading ? (
              ...
              <tr key={String(row.id)} className="border-t border-gray-100 hover:bg-gray-50">
                ...
                <td className="px-4 py-3">
                  <Link href={`/admin/blog/${row.id}`} className="text-indigo-600 hover:underline">
                    Edit
                  </Link>
                </td>
              </tr>
