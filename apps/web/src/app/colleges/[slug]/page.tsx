...
import EnrollmentChart from "@/components/college/EnrollmentChart";
...
function formatPackage(n: number | null): string | null {
  if (!n) return null;
...
export default async function CollegeDetailPage({
  ...
  const { data: cutoffsRaw } = await supabase
    ...
    .select("*, exams(name, slug), college_courses(course_name)")
    .eq("college_id", college.id)
    .order("year", { ascending: false });

  const cutoffs: any[] = cutoffsRaw ?? [];

  // Shaped for the trend chart — only series with 2+ real years render anything
  const cutoffTrendData = cutoffs.map((c: any) => ({
    year: c.year,
    category: c.category,
    cutoff_score: c.cutoff_score,
    cutoff_rank: c.cutoff_rank,
    exam_name: c.exams?.name ?? null,
    course_name: c.college_courses?.course_name ?? null,
  }));
  ...
  const naacColor = college.naac_grade
    ? (NAAC_COLORS[college.naac_grade] ?? "bg-gray-100 text-gray-600")
    : "";

  const bestKnownFor: string[] = Array.isArray(college.best_known_for)
    ? college.best_known_for
    : [];
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      {/* Accent bar */}
      <div
        className={`h-1.5 ${isTopRanked ? "bg-gradient-to-r from-yellow-400 to-orange-500" : "bg-gradient-to-r from-indigo-500 to-purple-600"}`}
      />
      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="bg-white rounded-2xl border border-gray-200 p-6 mb-6 shadow-sm">
          <div className="flex flex-col sm:flex-row sm:items-start gap-4">
            {/* Logo */}
            {college.logo_url && (
              <div className="flex-shrink-0 w-16 h-16 rounded-xl border border-gray-100 bg-white flex items-center justify-center overflow-hidden shadow-sm">
            ...
            <div className="flex-1">
              ...
              <div className="flex flex-wrap items-center gap-2 mb-3">
                ...
                {college.naac_grade && (
                  <span
                    className={`text-xs font-semibold px-2.5 py-1 rounded-full ${naacColor}`}
                  >
          ...
          {/* Fees Banner */}
          {(college.annual_fees_min !== null ||
            college.annual_fees_max !== null) && (
        ...
        {(college.why_join || bestKnownFor.length > 0) && (
          <div className="bg-gradient-to-br from-indigo-600 to-purple-600 rounded-2xl p-6 mb-6 shadow-sm text-white">
            <h2 className="text-lg font-semibold mb-3 flex items-center gap-2">
              <Heart className="w-5 h-5" /> Why Join {college.name}?
            </h2>
            {college.why_join && (
            ...
            {bestKnownFor.length > 0 && (
              <div>
                <p className="text-xs font-semibold text-indigo-200 uppercase tracking-wider mb-2 flex items-center gap-1.5">
                  <Sparkles className="w-3.5 h-3.5" /> What it&apos;s best at
                </p>
                <div className="flex flex-wrap gap-2">
                  {bestKnownFor.map((tag: string) => (
                    <span
                      key={tag}
                      className="text-xs font-medium px-3 py-1.5 rounded-full bg-white/15 border border-white/20 backdrop-blur-sm"
                    >
                      {tag}
                    </span>
                  ))}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="space-y-5">
            ...
            {latestPlacement && (
              <div className="bg-white rounded-2xl border border-gray-200 p-6 shadow-sm">
                <h2 className="text-base font-semibold text-gray-900 mb-4 flex items-center gap-2">
                  <TrendingUp className="w-5 h-5 text-emerald-500" /> Placements
                  ({latestPlacement.year})
                </h2>
                <div className="grid grid-cols-2 gap-3 mb-3">
                  {latestPlacement.placement_rate && (
                    <div className="bg-emerald-50 rounded-lg p-3 text-center">
                      <div className="text-lg font-bold text-emerald-700">
                        {latestPlacement.placement_rate}%
                      </div>
                      <div className="text-xs text-emerald-500">Placed</div>
                    </div>
                  )}
                  {formatPackage(latestPlacement.average_package) && (
                  ...
                  {formatPackage(latestPlacement.median_package) && (
                    <div className="bg-gray-50 rounded-lg p-3 text-center">
                      <div className="text-lg font-bold text-gray-800">
                        {formatPackage(latestPlacement.median_package)}
                      </div>
                  ...
                  {formatPackage(latestPlacement.highest_package) && (
                    <div className="bg-amber-50 rounded-lg p-3 text-center">
                      <div className="text-lg font-bold text-amber-700">
                        {formatPackage(latestPlacement.highest_package)}
                      </div>
                      <div className="text-xs text-amber-500">Highest</div>
                    </div>
                  )}
                </div>
                {latestPlacement.top_recruiters && (
                  <div className="mt-2">
                    <p className="text-xs font-semibold text-gray-400 uppercase tracking-wider mb-2 flex items-center gap-1">
                      <Briefcase className="w-3 h-3" /> Top Recruiters
                    </p>
                ...
                {latestPlacement.source_url && (
                  <a
                    href={latestPlacement.source_url}
                    target="_blank"
                    rel="noopener noreferrer"
              ...
              <div className="space-y-2">
                <div className="flex justify-between text-sm">
                  ...
                  <span className="font-semibold text-green-800">
                    {formatFees(
                      college.annual_fees_min,
                      college.annual_fees_max,
                    )}
                  </span>
                </div>
                {college.fee_note && (
                ...
                <p className="text-xs text-green-500 mt-2">
                  * Fees are approximate. Check official website for exact
                  figures.
                </p>
              </div>
            </div>
              ...
              <div className="space-y-2">
                {[
                  ...
                  <Link
                    key={label}
                    href={href}
                    className="flex items-center gap-2 text-sm text-indigo-700 hover:text-indigo-900 transition-colors group"
                  >
