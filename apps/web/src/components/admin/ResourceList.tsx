...
export default function ResourceList({ resource }: { resource: ResourceKey }) {
  ...
  const load = async (search = q) => {
    setLoading(true)
    const params = new URLSearchParams({ table: def.table })
    ...
    <div>
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">Create, edit, or remove records shown to students.</p>
        </div>
        <Link
          href={`/admin/${resourcePath}/new`}
            ...
            {loading ? (
              ...
              <tr key={String(row.id)} className="border-t border-gray-100 hover:bg-gray-50">
                ...
                <td className="px-4 py-3">
                  <Link href={`/admin/${resourcePath}/${row.id}`} className="text-indigo-600 hover:underline">
                    Edit
                  </Link>
                </td>
              </tr>
